...
function preVisualizarPagamentosInter_(periodo, telegram, meta) {
  ...
  if (!mensagens.length) {
    return {
      ...
      duplicados: [],
      ja_processados: [],
      message: "Nenhum e-mail de pagamentos do Inter encontrado para o período informado."
    };
  }
  ...
  for (let i = 0; i < mensagens.length; i++) {
    ...
    try {
      ...
      const tipoPagamento = payNorm_(extraido.tipo_pagamento || "");
      if (
        ["pix_realizado", "pix_qrcode", "debito_compra"].indexOf(tipoPagamento) < 0
      ) {
        ignorados.push({
      ...
      const dataPagamento = Utilities.formatDate(
        msg.getDate(),
        Session.getScriptTimeZone(),
        "yyyy-MM-dd"
      );
      ...
      const pagamento = {
        ...
        banco_email: String(extraido.banco_email || "Inter Empresas").trim(),
      ...
      const chaveDuplicidade = payMontarChaveDuplicidadeInter_(pagamento, msg);
