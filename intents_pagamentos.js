...
function isPagamentosIntent(text = "", message = {}) {
  const t = normalizeText(text);

  if (extractSpecificDates(text).length > 0 && t.includes("pag")) {
    return true;
  }
...
function parsePagamentosIntent(text = "", message = {}) {
  const t = normalizeText(text);

  let origem = "inter";
  if (t.includes("itau") || t.includes("itaú")) {
    origem = "itau";
  }
